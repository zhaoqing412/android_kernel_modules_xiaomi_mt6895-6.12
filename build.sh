...
#   4. out-of-tree modules -> 132 x .ko (make M=)
...
#   7. vendor_boot_new.img mkbootimg: official ramdisk with 132 x 6.12 .ko, DTBO_TAG
#                        dtb slot, vrt name=[]/type=[platform], padded 64MB
#   8. dtbo_new.img       DTOv1 single entry (xaga.dtbo), NOT padded
#
# --modules-only: compile modules only (config + in-tree + 132 .ko hard assert),
...
modules() {
    ...
    [ "$NKO" -eq 132 ] || { echo "ERROR: expected 132 .ko, got $NKO" >&2; exit 1; }
    echo "$NKO .ko built"
}
...
# ---------------------------------------------------------------------------
# 7. vendor_boot_new.img: official ramdisk, 5.10 .ko removed, 132 x 6.12 .ko
# ---------------------------------------------------------------------------
pack_vendor() {

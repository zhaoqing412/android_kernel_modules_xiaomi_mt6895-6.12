...
/* 6.12: g_esd_ctx no longer exported by mtk_disp_recovery; use set_panel_dead_flag */
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	ret = mtk_disp_notifier_register("Touch_driver", &ts->drm_notif);
	if(ret) {
...
#ifdef MI_DRM_NOTIFIER
	if (mtk_disp_notifier_unregister(&ts->drm_notif))

...
4. OOT 模块 → **134 个 `.ko`**（`make M=`，硬断言，含 xaga-marker-writer 诊断模块）
...
7. `images/out/vendor_boot_new.img` — mkbootimg：官方 vendor ramdisk 换 134 个 6.12 .ko（含 xaga-marker-writer 诊断模块，modules.load 排最前）+ 元数据重建（modules.load/recovery 134 行 + depmod 平铺）+ DTBO_TAG dtb 槽 + pad 64MB
8. `images/out/dtbo_new.img` — DTOv1 单条目（xaga.dtbo，不 pad）

```
./build.sh                        # 默认：全量编译 + 打包（产物 → images/out/）
./build.sh --modules-only         # 只编译内核模块（配置 → 134 个 .ko），不编译 Image/DTS、不打包
...
- 模块树内 **134 个 `.ko`**（`kernel_device_modules-6.12/` 原位）
...
- `build.sh` 一键构建脚本（clean → 配置 → in-tree 模块 → 134 个 .ko）
